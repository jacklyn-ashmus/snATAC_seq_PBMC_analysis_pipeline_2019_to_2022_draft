...
IN_PEAK_DIR=$1 # input directory with the fixed peak files from running bedops on summitsbed 
...
FIXED_PEAK_FILE_SUFFIX=$7 # .fixed_peaks.200bp.bed
...
for cell in "${celltypes[@]}"; do
    in_narrowpeak_file="${IN_PEAK_DIR}${cell}${FIXED_PEAK_FILE_SUFFIX}"
    out_bed_file="${OUT_PEAK_DIR}${cell}.fixed_peaks.bared.bed"
    echo "${out_bed_file}"
...
for cell in "${celltypes[@]}"; do
    bedfile="${OUT_PEAK_DIR}${cell}.fixed_peaks.bared.bed"
    cat "${bedfile}"
done > ${cat_file}
