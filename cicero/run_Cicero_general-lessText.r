...
sc.umap <- read.table( metafile, sep='\t',  header=T, row.names=1)
# print(colnames(sc.umap))
...
cicero_cds <- suppressWarnings(make_cicero_cds(input_cds, reduced_coordinates = umap_coords, k=30))
# print("")
# print("")
...
distance_parameters <- suppressWarnings(estimate_distance_parameter(cicero_cds, window=window, maxit=100, sample_num=100, distance_constraint=500000, genomic_coords=human.hg19.genome))
...
cicero_out <- suppressWarnings(generate_cicero_models(cicero_cds, distance_parameter=mean_distance_parameter, window=window, genomic_coords=human.hg19.genome))
