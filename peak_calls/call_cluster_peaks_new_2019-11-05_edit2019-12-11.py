...
def split_reads_and_call_peaks(args, c):
	...
	cluster_tagalign_presplit = args.multipleTagAlignDirectory + c + ".tagAlign.gz"
	cluster_barcodes = [bc for bc in args.cluster_map if args.cluster_map[bc]==c]
	if not os.path.isfile(cluster_tagalign) and not os.path.isfile(cluster_tagalign_presplit):
		print("\t" + c + " :  split tagalign file doesn't exist, creating tagalign file : " + cluster_tagalign)
		...
		if os.path.isfile(cluster_tagalign_presplit):
			print("\t" + c + " :  pre-split tagalign file " + cluster_tagalign_presplit + " exists")
			cluster_tagalign = cluster_tagalign_presplit
	print("\t" + c + " :  calling macs2")
	macs2_cmd = ['macs2', 'callpeak', '-t', cluster_tagalign, '--outdir', os.getcwds(), '-n', cluster_prefix, '-q', '0.05', '--nomodel', '--keep-dup', 'all', '-g', 'hs']
	...
	print("\t" + c + " :  total_reads = " + str(total_reads))
	print("\t" + c + " :  calling bedtools genomecov")
	genomecov_cmd = ['bedtools', 'genomecov', '-i', cluster_tagalign, '-bg', '-scale', '{}'.format(1e6/int(total_reads.decode())), '-g', args.genome_file]

	with open(args.output_prefix + '.scale_1e6.bdg', 'w') as f:
		subprocess.call(genomecov_cmd, stdout=f)
	print("\t" + c + " :  making bigwig")
	subprocess.call(['bedGraphToBigWig', args.output_prefix + '.scale_1e6.bdg', args.genome_file, args.output_prefix + '.scale_1e6.ATAC.bw'])
...
def process_args():
	...
	io_group.add_argument('-m', '--multipleTagAlignDirectory', required=False, default='', type=str, help='EDIT: existing multiple tagalign directory and file prefix')
