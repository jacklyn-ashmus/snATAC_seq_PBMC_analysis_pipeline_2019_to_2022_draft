...
matrixFileOutputPrefix=/nfs/lab/jnewsome/pbmc/mtx_finePops/pbmc1-15.
...
bdgFilePrefix=/nfs/lab/jnewsome/pbmc/bdg/pbmc1_15.
bdgFileSuffix=.scale_1e6.bdg
bdgSortedFileSuffix=.scale_1e6.sorted.bdg

bwFilePrefix=/nfs/lab/jnewsome/pbmc/bw/pbmc1_15.
...
mergedPeaksFileSuffix=.merged_peaks.anno.bed
tagalignSortedFilePrefix=/nfs/lab/jnewsome/pbmc/splitTagalign2021/pbmc1-15.
tagalignSortedFileSuffix=.filt.rmdup.2021-08-08.sort.tagAlign
...
for cell in ${cells[@]}; do 
    ...
    size=$(cat "$fileSizeFile")  
    SCALE_FACTOR=1e6/${size}
    dt=$(date '+%d/%m/%Y %H:%M:%S');
    echo "${cell} total reads = ${size}      $dt"
    echo "${cell} scale factor = ${SCALE_FACTOR}      $dt"
